...
void MainWindow::update_map_image() {
    ...
    for (const auto& fl : flowers) { // Draw all the flowers onto the image
        draw_flower(image, fl);
    }
    scene->addPixmap(QPixmap::fromImage(image)); // Add the image to the scene
}

void MainWindow::draw_flower(QImage& img, const flower& fl) { // Re-draw the flower onto the image with the updated xy-coordinates
    // Redraw the specific flower for the next generations
    for (int dx = -flower_size; dx <= flower_size; dx++) {
        for (int dy = -flower_size; dy <= flower_size; dy++) {
            // Check if the current pixel is within the circle
            if (dx * dx + dy * dy <= pow(flower_size, 2)) {
                ...
                img.setPixel(fl.xy_cor[0] + dx, fl.xy_cor[1] + dy, qRgb(255 * fl.corolla_size / max_corolla_size, 0, 0));
            }
        }
    }
}
